  ...
  const normalizedMovie = useMemo(() => {
    ...
    return {
      ...
      genres: Array.isArray(movie.genres) ? movie.genres : 
             (typeof movie.genres === 'string' ? movie.genres.split(',').map(g => g.trim()) : []),
  ...
  useEffect(() => {
    ...
    const getPosterUrl = async () => {
      ...
      try {
        ...
        const y = normalizedMovie.release_date ? new Date(normalizedMovie.release_date).getFullYear() : null;
        if (!t) return;
      ...
      <div className="mt-4 bg-white p-6 rounded-lg shadow relative">
        ...
        <div className="mt-4 grid gap-6 md:grid-cols-3">
          ...
          <div className="md:col-span-1 md:sticky md:top-24 self-start">
            <div className="relative">
              {posterUrl ? (
                <div className="mx-auto w-full max-w-[200px] sm:max-w-[220px] md:max-w-[240px] rounded-md overflow-hidden aspect-[2/3] bg-gray-100 relative">
                  ...
                  <img
                    src={posterUrl}
                    alt={title}
                    className={`h-full w-full object-cover ${posterLoading ? 'opacity-0' : 'opacity-100'}`}
                    loading="lazy"
                    onError={(e) => {
              ...
              {vote_average > 0 && (
                <div className="mt-3 flex items-center justify-center">
                  <div className="flex items-center bg-yellow-50 text-yellow-700 px-2 py-1 rounded-full text-sm">
                    ...
                    {vote_count > 0 && (
                      <span className="text-xs text-gray-500 ml-1">({vote_count})</span>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                {release_date && (
                  <div className="flex">
                    <span className="w-24 font-medium text-gray-900">Release:</span>
                ...
                {status && status !== 'Released' && (
                  <div className="flex">
                    <span className="w-24 font-medium text-gray-900">Status:</span>
                ...
                {runtime > 0 && (
                  <div className="flex">
                    <span className="w-24 font-medium text-gray-900">Runtime:</span>
          ...
          <div className="md:col-span-2">
            ...
            {cast && cast.length > 0 && (
              <div className="mb-4">
                <h3 className="text-md font-semibold text-gray-900 mb-2">Cast</h3>
                <div className="flex flex-wrap gap-2">
                  {cast.slice(0, 10).map((person, i) => (
                    <span 
                      key={`cast-${i}`}
                      className="inline-flex items-center rounded-full bg-gray-50 px-2.5 py-0.5 text-xs font-medium text-gray-800"
                    >
                      {person}
                    </span>
                  ))}
            ...
            {crew && crew !== 'N/A' && (
              <div>
                <h3 className="text-md font-semibold text-gray-900 mb-2">Crew</h3>
                <p className="text-sm text-gray-700">
                  {Array.isArray(crew) ? crew.join(', ') : crew}
                </p>
              </div>
            )}
          </div>
