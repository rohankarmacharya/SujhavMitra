...
import { fetchBookById, fetchBookByTitle, fetchBookRecommendations } from "../services/api";
...
const BookDetails = () => {
  ...
  // Build cover image URL from ISBN (Open Library Covers API)
  useEffect(() => {
    const i = (book?.isbn || "").toString().trim();
    if (!i) {
      setCoverUrl(null);
      return;
    }
    // Use large size if available; this is a simple, keyless API
    setCoverUrl(`https://covers.openlibrary.org/b/isbn/${encodeURIComponent(i)}-L.jpg`);
  }, [book?.isbn]);

  // Legacy localStorage-based wishlist is superseded by useWishlist (heart & button both use it)
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-sm rounded-xl">
      <button
        onClick={() => navigate(-1)}
      ...
      <div className="mt-2 grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2">
          <div className="flex items-start justify-between gap-3">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2 break-words">{book.title}</h1>
            <button
          ...
          {book.author && <p className="text-base md:text-lg text-gray-800 mb-1"><span className="font-medium text-gray-900">Author:</span> {book.author}</p>}
      ...
      <section className="mt-10">
        <h2 className="text-xl md:text-2xl font-semibold text-gray-900 mb-4">Related Books</h2>
        {loadingRelated ? (
          <Loading />
