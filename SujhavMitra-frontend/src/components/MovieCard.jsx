...
export default function MovieCard({ movie }) {
  ...
  const { id, title, overview, genres, cast, year } = movie;
      ...
      <Link to={`/movie/${id}`} state={{ movie }} className="block h-full">
        <CardContent className="flex h-full flex-col">
          <div className="absolute right-2 top-2 z-10">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`h-5 w-5 ${saved ? "" : ""}`}>
          ...
          <div className="mb-3 overflow-hidden rounded-md bg-gray-100 aspect-[2/3] relative">
            ...
            {posterUrl ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-gray-500 text-sm" onLoad={() => setPosterLoading(false)}>No image</div>
            )}
          ...
          <CardTitle title={title} className="text-base md:text-xl break-words line-clamp-2">{title}</CardTitle>
          ...
          {overview && (
            <p className="mt-2 line-clamp-3 text-sm leading-6 text-gray-800">{overview}</p>
          )}
          {Array.isArray(cast) && cast.length > 0 && (
            <p className="mt-2 truncate text-xs text-gray-600">Cast: {cast.slice(0, 4).join(", ")}</p>
          )}
